namespace Klasa_enkapsulacja
{
    public class Product
    {
        ...
        public void ObliczCenePoRabacie(decimal procentRabatu)
        {
            if (procentRabatu < 0 || procentRabatu > 100)
            {
                throw new ArgumentException("Procent rabatu musi być między 0 a 100.");
            }
            decimal rabat = (procentRabatu / 100) * Cena;
            decimal cenaPoRabacie = Cena - rabat;
            Console.WriteLine($"Cena po rabacie {procentRabatu}%: {cenaPoRabacie} PLN");
        }

        public Boolean czyDostepny(int iloscNaMagazynie)
        {
            if (iloscNaMagazynie > 0)
            {
                Console.WriteLine("Produkt jest dostępny");
                return true;

            }
            else
            {
                Console.WriteLine("Produkt jest niedostępny.");
                return false;
            }
        }
    }
